#include "pubghypetrain.h"
#include <iostream>

void collide(int posX, int posY, int &velX, int &velY, int xmax, int ymax);

void moveBall(int &posX, int &posY, int velX, int velY);

void draw(int posX, int posY, int xmax, int ymax, const char* syms);

int main()
{
  const char SYMBOLS[2] = {'.','O'};
  int T_SLEEP_MILLIS = 50; 
  int GLOBAL_H = 10;
  int GLOBAL_W = 10;
  int posX = 5;
  int posY = 2;
  int velX = 2;
  int velY = -1;
  for(int i = 0; i < 50; i++){
    moveBall(posX,posY,velX,velY);
    collide(posX,posY,velX,velY,GLOBAL_H,GLOBAL_W);
    draw(posX,posY,GLOBAL_H,GLOBAL_W,SYMBOLS);
    sleep(T_SLEEP_MILLIS);
  };
  return 0;
};


void moveBall(int &posX, int &posY, int velX, int velY){
  posX += velX;
  posY += velY;
};

void collide(int posX, int posY, int &velX, int &velY, int xmax, int ymax){
  velX += -2 * velX * (posX <= 0 || posX >= xmax-1);
  velY += -2 * velY * (posY <= 0 || posY >= ymax-1);
};

void draw(int posX, int posY, int xmax, int ymax, const char* syms){
  ClearScreen();
  for(int y = 0; y < ymax; y++){
    for(int x = 0; x < xmax; x++){
      std::cout << syms[(posX == x) && (posY == y)];
    };
    std::cout << std::endl;
  };
};