...
public:
	...
	SDLTTF()
  {
    if (TTF_Init() != 0)throw_sdl("TTF_Init");
  }

	virtual ~SDLTTF(){TTF_Quit();}
...
public:
	...
	if ((font = TTF_OpenFontIndex(file.c_str(), ptsize, index)) == nullptr)throw_sdl("TTF_OpenFontIndex");
}
	...
	TTF_Font* get() const
  {
	...
	Surface render_txt_solid(const std::string& text, SDL_Color fg)
  {
    SDL_Surface* surface = TTF_RenderText_Solid(font_, text.c_str(), fg);
    if (surface == nullptr)throw_sdl("TTF_RenderText_Solid");
