...
class Unirand {
    ...
    public:
        Unirand(long long seed, int min, int max): random(
            std::bind(
...
public:
    ...
    void rmean()
    {
      msum = mcount = 0;
      rmean(root);
      mean = static_cast<double>(msum)/static_cast<double>(mcount);
      msum = mcount = 0;
    }
    
    void rmean(Node* node)
    {
      if(node)
      {
          ++depth;
    ...
    bool operator<(const BinRandTree& o) const
    {
      return getVar() < o.getVar();
    }
